...
"""
GraphGPO Recipe: FSDP PPO Trainer with Ray-based single controller.
Self-contained recipe. Builds a state-transition graph from rollout trajectories
and uses shortest-path distances to compute step-level returns.
"""
...
from recipe.GraphGPO.rollout_loop import TrajectoryCollector
from recipe.GraphGPO.utils import adjust_batch
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, episode_advantage_w=0.0, gigpo_mode="mean_std_norm", **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.GraphGPO:
        advantages, returns = core_graph.compute_graphgpo_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            step_rewards=data.batch['step_rewards'],
            response_mask=data.batch['response_mask'],
            anchor_obs=data.non_tensor_batch['anchor_obs'],
...
class RayPPOTrainer:
    ...
    def _maybe_dump_val_trajectories(self, batch, batch_idx):
        """Dump validation step-level trajectories when explicitly configured."""
        dump_dir = self.config.trainer.get("val_trajectory_dump_dir", None)
        if not dump_dir:
            return

        os.makedirs(dump_dir, exist_ok=True)
        filename = os.path.join(dump_dir, f"step_{self.global_steps}_batch_{batch_idx}.jsonl")
        keys = [
            "uid",
            "traj_uid",
            "anchor_obs",
        ...
        rows = len(batch.non_tensor_batch.get("traj_uid", []))
        with open(filename, "w") as f:
            for i in range(rows):
                entry = {"global_step": self.global_steps, "batch_idx": batch_idx, "row": i}
                for key in keys:
                    if key in batch.non_tensor_batch:
                        entry[key] = scalarize(batch.non_tensor_batch[key][i])
                f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    ...
    def _validate(self):
        ...
        for batch_idx, test_data in enumerate(self.val_dataloader):
            ...
            self._maybe_dump_val_trajectories(test_batch, batch_idx)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    elif self.config.algorithm.adv_estimator == AdvantageEstimator.GraphGPO:
                        graphgpo_cfg = self.config.algorithm.get("graphgpo", {})
                        Gs = core_graph.ComplexGraph.from_data(
                            gen_batch_output,
                            enable_similarity=graphgpo_cfg.get("enable_similarity", False),
                            similarity_thresh=graphgpo_cfg.get("similarity_thresh", 0.95),
                        )
                        core_graph.ComplexGraph.Calculate_all_shortest_path(Gs)
                        step_rewards_tensor = core_graph.compute_graph_path_returns(
                            batch=batch,
                            Gs=Gs,
                            gamma=self.config.algorithm.gamma,
                            normalize_distance=graphgpo_cfg.get("normalize_distance", False),
                        )
                        batch.batch['step_rewards'] = step_rewards_tensor
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            step_advantage_w=self.config.algorithm.get("graphgpo", {}).get("step_advantage_w", 1.0),
                            episode_advantage_w=self.config.algorithm.get("graphgpo", {}).get("episode_advantage_w", 1.0),
                            gigpo_mode=self.config.algorithm.get("graphgpo", {}).get("mode", "mean_std_norm"),
                            epsilon=1e-6,
                        )
