...
class TrajectoryCollector:
    ...
    def gather_rollout_data(
        ...
        for bs in range(batch_size):
            ...
            for _,data in enumerate(total_batch_list[bs]):
                ...
                if data['active_masks']:
                    ...
                    if (episode_rewards[bs]>0) and (_==len(total_batch_list[bs])-1):
                        data['next_obs']='success'
                    effective_batch.append(data)
                else:
                    if (episode_rewards[bs]>0) and (total_batch_list[bs][_-1]['active_masks']):
                        effective_batch[-1]['next_obs']='success'
    ...
    def vanilla_multi_turn_loop(
        ...
        if "index" in gen_batch.non_tensor_batch:
            ...
            env_kwargs["env_indices"] = gen_batch.non_tensor_batch["index"]
        ...
        for _step in range(self.config.env.max_steps):
            ...
            next_obs_=next_obs.get('anchor', None)
            batch.non_tensor_batch['next_obs'] = torch_to_numpy(next_obs_, is_object=True) if isinstance(next_obs_, torch.Tensor) else next_obs_


            # for _ in range(len(batch.non_tensor_batch['next_obs'])):
            #     if batch.non_tensor_batch['next_obs'][_] in ['Nothing happens.']:
            #         batch.non_tensor_batch['next_obs'][_]=batch.non_tensor_batch['anchor_obs'][_]+batch.non_tensor_batch['next_obs'][_]

            # for _ in range(len(batch.non_tensor_batch['next_obs'])):
            #     if isinstance(batch.non_tensor_batch['next_obs'][_],str):
            #         if batch.non_tensor_batch['next_obs'][_] in ['Nothing happens.','You are not carrying anything.', 'You can']:
            #             batch.non_tensor_batch['next_obs'][_]=batch.non_tensor_batch['anchor_obs'][_]
                        # batch.non_tensor_batch['is_action_valid'][_]=0

            # for _ in range(len(batch.non_tensor_batch['next_obs'])):
            #     if batch.non_tensor_batch['next_obs'][_] in ['Nothing happens.Nothing happens.']:
            #         batch.non_tensor_batch['is_action_valid'][_]=0            
