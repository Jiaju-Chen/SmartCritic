...
class AlfworldWorker:
    ...
    def reset_to_gamefile(self, gamefile):
        """Reset this worker to a specific ALFWorld game file for deterministic evaluation."""
...
class AlfworldEnvs(gym.Env):
    def __init__(self, alf_config_path, seed, env_num, group_n, resources_per_worker, is_train=True, env_kwargs={}):
        ...
        self.game_files = list(base_env.game_files)
    ...
    def reset_to_indices(self, indices):
        """
        Reset workers to explicit game indices.

        This is used only for evaluation so validation batches map exactly to the
        parquet extra_info.index values instead of relying on each TextWorld
        worker's independent game iterator.
        """
        assert len(indices) == self.num_processes, \
            "The num of indices must be equal to the num of processes"

        futures = []
        for worker, index in zip(self.workers, indices):
            gamefile = self.game_files[int(index) % len(self.game_files)]
            future = worker.reset_to_gamefile.remote(gamefile)
            futures.append(future)
